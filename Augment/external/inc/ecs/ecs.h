...
namespace ecs{
	...
	public:
		...
		static void CallFunction(F& func, unsigned int entityIndex, std::array<Pool*, MAX_COMPONENTS>& pools, Args... args){
			UnpackHelper<TComp...>::CallFunction(func, entityIndex, pools, args..., (T*)pools[T::componentId]->Get(entityIndex));
		}
	...
	void EntityManager::UpdateEntities(F& f, unsigned int start, unsigned int end){
		...
		for (; start != end && start < entityCount; ++start){
			if ((mask & entityMasks[start]) == mask){
				UnpackHelper<TComp...>::CallFunction(f, start, componentPools);// , pools.size() - 1);
			}
		}
	}
